...
export default function Home() {
  ...
  const handleUpdateTask = (taskId, patch) => {
    setStage(curr => {
      const boxId = Object.keys(curr).find(id => curr[id].items.some(item => item.id === taskId));
      ...
      return {
        ...
        [boxId]: {
          ...
          items: curr[boxId].items.map(item => (
            item.id === taskId ? { ...item, ...patch, updatedAt: nowAsStamp() } : item
          ))
        }
      }
    })
  ...
  return (
    ...
    >
      <main
        className="flex items-start justify-center pt-10 gap-10"
      >
